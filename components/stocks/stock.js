...
function Stock({ props: { symbol, price, delta } }) {
    ...
    const [priceAtTime, setPriceAtTime] = (0, react_1.useState)({
        time: '00:00',
        value: price.toFixed(2),
        x: 0
    });
    const [startHighlight, setStartHighlight] = (0, react_1.useState)(0);
    const [endHighlight, setEndHighlight] = (0, react_1.useState)(0);
    const chartRef = (0, react_1.useRef)(null);
    const { width = 0 } = useResizeObserver(chartRef);
    const xToDate = scaleLinear([0, width], [(0, utils_1.subMonths)(new Date(), 6).getTime(), new Date().getTime()]);
    ...
    (0, react_1.useEffect)(() => {
        if (startHighlight && endHighlight) {
            const message = {
                id,
                role: 'system',
                content: `[User has highlighted dates between between ${(0, utils_1.format)(new Date(xToDate(startHighlight)), 'd LLL')} and ${(0, utils_1.format)(new Date(xToDate(endHighlight)), 'd LLL, yyyy')}`
            };
    ...
    return (<div className="rounded-xl border bg-zinc-950 p-4 text-green-400">
      ...
      <div className="relative -mx-4 cursor-col-resize" onPointerDown={event => {
            if (chartRef.current) {
                const { clientX } = event;
                ...
                setEndHighlight(0);
                setPriceAtTime({
                    time: (0, utils_1.format)(new Date(xToDate(clientX)), 'dd LLL yy'),
        ...
        {priceAtTime.x > 0 ? (<div className="pointer-events-none absolute z-10 flex w-fit select-none gap-2 rounded-md bg-zinc-800 p-2" style={{
        ...
        {startHighlight ? (<div className="pointer-events-none absolute h-32 w-5 select-none rounded-md border border-zinc-500 bg-zinc-500/20" style={{
                left: startHighlight,
                width: endHighlight
